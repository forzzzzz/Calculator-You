...
import com.forz.calculator.old_arch.expression.ExpressionEditText
import com.forz.calculator.old_arch.calculator.AdditionalOperator
import com.forz.calculator.old_arch.calculator.Bracket
import com.forz.calculator.old_arch.calculator.Constant
import com.forz.calculator.old_arch.calculator.DefaultOperator
import com.forz.calculator.old_arch.calculator.ScientificFunction
import com.forz.calculator.old_arch.calculator.TrigonometricFunction
...
object InsertInExpression {
    ...
    fun enterBackspace(inputEditText: ExpressionEditText) {
        ...
        if (cursorPositionStart != cursorPositionEnd) {
            ...
            if (stringBeforeCursor.endsWith(Config.groupingSeparatorSymbol)){
                deleteSelection(inputEditText, cursorPositionStart - 1 - Config.groupingSeparatorSymbol.length, cursorPositionStart)
        ...
        if (inputOperator != DefaultOperator.Minus.text){
            ...
            val operator = if (
                (beforeDigitArray.any { stringBeforeCursor.endsWith(it) }
                        ...
                        || stringBeforeCursor.endsWith(Config.groupingSeparatorSymbol))
    ...
    fun enterScienceFunction(inputScienceFunction: String, inputEditText: ExpressionEditText) {
        ...
        if (!stringBeforeCursor.endsWith(Config.decimalSeparatorSymbol)){
    ...
    fun enterDot(inputEditText: ExpressionEditText) {
        ...
        if (lastCharIsDigit && !checkPoint(string, cursorPositionStart) && string.isNotEmpty()) {
            inputEditText.text?.insert(cursorPositionStart, Config.decimalSeparatorSymbol)
        } else if (string.isEmpty() || lastCharIsOperator){
            inputEditText.text?.insert(cursorPositionStart, "0${Config.decimalSeparatorSymbol}")
        } else if (lastCharIsAdditionalOperator){
            inputEditText.text?.insert(cursorPositionStart, "${DefaultOperator.Multiply.text}0${Config.decimalSeparatorSymbol}")
        }
    }
    ...
    private fun checkPoint(inputText: String, cursorPosition: Int): Boolean {
        ...
        val newText = NumberFormatter.removeSeparators(inputText, Config.groupingSeparatorSymbol)
        ...
        outerLoop@ for (i in subTextBefore.length - 1 downTo 0) {
            if (subTextBefore[i].toString() == Config.decimalSeparatorSymbol) {
                foundDigitBefore = true
                break@outerLoop
            } else if (!subTextBefore[i].isDigit() && subTextBefore[i].toString() != Config.decimalSeparatorSymbol) {
                break@outerLoop
            }
        }

        outerLoop@ for (i in subTextAfter.indices) {
            if (subTextAfter[i].toString() == Config.decimalSeparatorSymbol) {
                foundDigitAfter = true
                break@outerLoop
            } else if (!subTextAfter[i].isDigit() && subTextAfter[i].toString() != Config.decimalSeparatorSymbol) {
                break@outerLoop
            }
        }
